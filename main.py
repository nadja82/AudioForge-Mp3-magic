...
class MainWindow(QMainWindow):
    ...
    # Praxisnahe Export-Profile für die gängigsten Audioformate.
    # Die Optionen werden direkt vor die Ausgabedatei in die FFmpeg-Argumente
    # eingefügt. Dadurch überschreiben sie ältere Defaults aus core.ffmpeg_runner,
    # ohne dass dafür andere Projektdateien angepasst werden müssen.
    FORMAT_QUALITY_PROFILES = {
        "MP3": [
            {"id": "mp3-v0", "label": "V0 VBR – beste MP3-Qualität", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libmp3lame", "-q:a", "0", "-compression_level", "0"]},
            {"id": "mp3-v2", "label": "V2 VBR – sehr gut / kleiner", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libmp3lame", "-q:a", "2", "-compression_level", "0"]},
            {"id": "mp3-320", "label": "320 kbit/s CBR – maximale Kompatibilität", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libmp3lame", "-b:a", "320k", "-compression_level", "0"]},
            {"id": "mp3-256", "label": "256 kbit/s CBR", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libmp3lame", "-b:a", "256k", "-compression_level", "0"]},
            {"id": "mp3-192", "label": "192 kbit/s CBR", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libmp3lame", "-b:a", "192k", "-compression_level", "0"]},
            {"id": "mp3-128", "label": "128 kbit/s CBR – klein", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libmp3lame", "-b:a", "128k", "-compression_level", "2"]},
        ],
        "AAC": [
            {"id": "aac-320", "label": "AAC 320 kbit/s – beste Qualität", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "aac", "-b:a", "320k"]},
            {"id": "aac-256", "label": "AAC 256 kbit/s – sehr gut", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "aac", "-b:a", "256k"]},
            {"id": "aac-192", "label": "AAC 192 kbit/s", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "aac", "-b:a", "192k"]},
            {"id": "aac-128", "label": "AAC 128 kbit/s – klein", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "aac", "-b:a", "128k"]},
            {"id": "aac-96", "label": "AAC 96 kbit/s – Sprache/klein", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "aac", "-b:a", "96k"]},
        ],
        "OPUS": [
            {"id": "opus-192", "label": "Opus 192 kbit/s VBR – transparent", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libopus", "-b:a", "192k", "-vbr", "on", "-compression_level", "10", "-application", "audio"]},
            {"id": "opus-128", "label": "Opus 128 kbit/s VBR – sehr gut", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libopus", "-b:a", "128k", "-vbr", "on", "-compression_level", "10", "-application", "audio"]},
            {"id": "opus-96", "label": "Opus 96 kbit/s VBR – kompakt", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libopus", "-b:a", "96k", "-vbr", "on", "-compression_level", "10", "-application", "audio"]},
            {"id": "opus-64", "label": "Opus 64 kbit/s VBR – sehr klein", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libopus", "-b:a", "64k", "-vbr", "on", "-compression_level", "10", "-application", "audio"]},
        ],
        "OGG": [
            {"id": "vorbis-q10", "label": "Vorbis q10 – beste Qualität", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libvorbis", "-q:a", "10"]},
            {"id": "vorbis-q8", "label": "Vorbis q8 – sehr gut", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libvorbis", "-q:a", "8"]},
            {"id": "vorbis-q6", "label": "Vorbis q6 – gut", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libvorbis", "-q:a", "6"]},
            {"id": "vorbis-q4", "label": "Vorbis q4 – kleiner", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "libvorbis", "-q:a", "4"]},
        ],
        "FLAC": [
            {"id": "flac-8", "label": "Lossless FLAC Level 8 – Archiv", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "flac", "-compression_level", "8"]},
            {"id": "flac-5", "label": "Lossless FLAC Level 5 – Standard", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "flac", "-compression_level", "5"]},
            {"id": "flac-0", "label": "Lossless FLAC Level 0 – schnell", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "flac", "-compression_level", "0"]},
        ],
        "ALAC": [
            {"id": "alac-lossless", "label": "Lossless ALAC – Apple Lossless", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "alac"]},
        ],
        "WAV": [
            {"id": "wav-24", "label": "Lossless WAV PCM 24-bit – Studio", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "pcm_s24le"]},
            {"id": "wav-16", "label": "Lossless WAV PCM 16-bit – CD/kompatibel", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "pcm_s16le"]},
            {"id": "wav-f32", "label": "Lossless WAV 32-bit Float – Bearbeitung", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "pcm_f32le"]},
        ],
        "AIFF": [
            {"id": "aiff-24", "label": "Lossless AIFF PCM 24-bit – Studio", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "pcm_s24be"]},
            {"id": "aiff-16", "label": "Lossless AIFF PCM 16-bit – kompatibel", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "pcm_s16be"]},
            {"id": "aiff-f32", "label": "Lossless AIFF 32-bit Float – Bearbeitung", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "pcm_f32be"]},
        ],
        "WAVPACK": [
            {"id": "wv-lossless", "label": "Lossless WavPack – Archiv", "core_quality": DEFAULT_QUALITY, "ffmpeg_args": ["-c:a", "wavpack"]},
        ],
    }

    LOSSLESS_FORMAT_KEYS = {"FLAC", "ALAC", "WAV", "AIFF", "WAVPACK"}
    ...
    def _build_ui(self) -> None:
        ...
        output_layout.addWidget(QLabel("Format:"),0,0); output_layout.addWidget(self.format_combo,0,1,1,2); output_layout.addWidget(QLabel("Qualität:"),1,0); output_layout.addWidget(self.quality_combo,1,1,1,2); output_layout.addWidget(QLabel("Abtastrate:"),2,0); output_layout.addWidget(self.sample_rate_combo,2,1,1,2); output_layout.addWidget(QLabel("Ausgabeordner:"),3,0); output_layout.addWidget(self.output_dir_edit,3,1); output_layout.addWidget(self.choose_output_btn,3,2); output_layout.addWidget(self.open_output_btn,4,2); output_layout.addWidget(self.preserve_structure_cb,5,0,1,3); output_layout.addWidget(self.auto_number_cb,6,0,1,3); output_layout.addWidget(self.keep_metadata_cb,7,0,1,3); right_layout.addWidget(output_box)
    ...
    def _load_settings_to_ui(self) -> None:
        ...
        self._refresh_quality_profiles_for_format(self.format_combo.currentText(), str(self.settings.value("quality_profile", "")))
        if not self._set_combo_by_data(self.sample_rate_combo, self.settings.value("sample_rate", "source")):
    ...
    def _connect_settings_signals(self) -> None:
        ...
        for combo in [self.quality_combo, self.sample_rate_combo, self.preset_combo]:
    ...
    def _set_combo_by_data(self, combo: QComboBox, value: object) -> bool:
    ...
    def _format_key(self, format_name: str) -> str:
        text = format_name.upper()
        if "WAVPACK" in text or text in {"WV", "WVPACK"}:
            return "WAVPACK"
        if "FLAC" in text:
            return "FLAC"
        if "ALAC" in text or "APPLE LOSSLESS" in text:
            return "ALAC"
        if "AIFF" in text or text == "AIF":
            return "AIFF"
        if "WAV" in text:
            return "WAV"
        if "OPUS" in text:
            return "OPUS"
        if "OGG" in text or "VORBIS" in text:
            return "OGG"
    ...
    def _profiles_for_format(self, format_name: str) -> list[dict[str, object]]:
        return list(self.FORMAT_QUALITY_PROFILES.get(self._format_key(format_name), []))

    def _current_quality_profile_id(self) -> str:
        data = self.quality_combo.currentData() if hasattr(self, "quality_combo") else None
        if isinstance(data, dict):
            return str(data.get("id", self.quality_combo.currentText()))
        return str(data or self.quality_combo.currentText())

    def _current_quality_profile(self) -> dict[str, object]:
        data = self.quality_combo.currentData() if hasattr(self, "quality_combo") else None
        if isinstance(data, dict):
            return data
        return {"id": self.quality_combo.currentText(), "label": self.quality_combo.currentText(), "core_quality": self.quality_combo.currentText(), "ffmpeg_args": []}

    def _refresh_quality_profiles_for_format(self, format_name: str, preferred_id: str | None = None) -> None:
        if not hasattr(self, "quality_combo"):
            return
        previous_id = preferred_id or self._current_quality_profile_id()
        self.quality_combo.blockSignals(True)
        self.quality_combo.clear()
        profiles = self._profiles_for_format(format_name)
        if profiles:
            for profile in profiles:
                self.quality_combo.addItem(str(profile["label"]), profile)
            if previous_id and self._set_combo_by_profile_id(previous_id):
                pass
            else:
                # Bei verlustfreien Formaten automatisch das erste Lossless-Profil wählen.
                self.quality_combo.setCurrentIndex(0)
        else:
            self.quality_combo.addItems(quality_options())
            self._set_combo_if_valid(self.quality_combo, str(self.settings.value("quality", DEFAULT_QUALITY)) if hasattr(self, "settings") else DEFAULT_QUALITY)
        self.quality_combo.blockSignals(False)

    def _set_combo_by_profile_id(self, profile_id: str) -> bool:
        for idx in range(self.quality_combo.count()):
            data = self.quality_combo.itemData(idx)
            if isinstance(data, dict) and str(data.get("id")) == str(profile_id):
                self.quality_combo.setCurrentIndex(idx)
                return True
        return False

    def _sample_rates_for_format(self, format_name: str) -> list[int]:
        key = self._format_key(format_name)
        if key == "MP3":
            return list(self.MP3_SAMPLE_RATES)
        if key == "AAC":
            return list(self.AAC_SAMPLE_RATES)
        if key == "OPUS":
            return list(self.OPUS_SAMPLE_RATES)
        return list(self.COMMON_SAMPLE_RATES)

    def _format_sample_rate(self, value: int | None) -> str:
        if not value:
            return "Wie Quelle/Analyse"
        return f"{value / 1000:g} kHz" if value >= 1000 else f"{value} Hz"

    def _refresh_sample_rate_options(self, format_name: str, preferred_value: object | None = None) -> None:
        if not hasattr(self, "sample_rate_combo"):
            return
        previous = preferred_value if preferred_value is not None else self.sample_rate_combo.currentData()
        self.sample_rate_combo.blockSignals(True)
        self.sample_rate_combo.clear()
        self.sample_rate_combo.addItem("Wie Quelle/Analyse", "source")
        for rate in self._sample_rates_for_format(format_name):
            self.sample_rate_combo.addItem(self._format_sample_rate(rate), rate)
        if not self._set_combo_by_data(self.sample_rate_combo, previous or "source"):
            self.sample_rate_combo.setCurrentIndex(0)
        self.sample_rate_combo.blockSignals(False)
        self._update_sample_rate_source_hint()

    def _update_sample_rate_source_hint(self) -> None:
        if not hasattr(self, "sample_rate_combo"):
            return
        rows = self.selected_rows_or_first()
        rate = self.jobs[rows[0]].sample_rate if rows else None
        text = "Wie Quelle/Analyse"
        if rate:
            text = f"Wie Quelle/Analyse ({self._format_sample_rate(rate)})"
        self.sample_rate_combo.setItemText(0, text)

    def on_output_format_changed(self, format_name: str) -> None:
        self._refresh_quality_profiles_for_format(format_name)
        self._refresh_sample_rate_options(format_name)
        self.save_settings()

    def _probe_sample_rate(self, path: Path) -> int | None:
        try:
    ...
    def _resolve_output_sample_rate(self, job: AudioJob, choice: object, format_name: str) -> int | None:
        if str(choice) == "source":
            rate = job.sample_rate or self._probe_sample_rate(job.source)
        else:
            try:
                rate = int(choice)
            except Exception:
        ...
        if not rate:
            return None
        allowed = self._sample_rates_for_format(format_name)
        # Manche Encoder akzeptieren nur bestimmte Raten. Wenn „Wie Quelle“ z.B.
        # 96 kHz bei MP3 wäre, wählen wir die nächste kompatible Rate, damit der
        # Export nicht abbricht.
        if rate not in allowed and self._format_key(format_name) in {"MP3", "AAC", "OPUS"}:
            return min(allowed, key=lambda candidate: abs(candidate - rate))
        return rate

    def _inject_output_audio_options(self, args: list[str], output_audio_options: list[str]) -> list[str]:
        if not output_audio_options or len(args) < 2:
            return args
        # FFmpeg-Output-Optionen gehören unmittelbar vor den Ausgabepfad.
        # Doppelte Optionen sind hier Absicht: die späteren Werte überschreiben
        # ältere Defaults aus build_ffmpeg_args.
        return [*args[:-1], *output_audio_options, args[-1]]
    ...
    def save_settings(self) -> None:
        ...
        self.settings.setValue("quality_profile", self._current_quality_profile_id())
        self.settings.setValue("sample_rate", self.sample_rate_combo.currentData() if hasattr(self, "sample_rate_combo") else "source")
    ...
    def reset_settings(self) -> None:
        ...
        self._refresh_quality_profiles_for_format("MP3", "mp3-v0")
        self._refresh_sample_rate_options("MP3")
        self._set_combo_by_data(self.sample_rate_combo, "source")
    ...
    def _add_paths(self, paths: list[Path]) -> None:
        ...
        for path in paths:
            ...
            self.jobs.append(AudioJob(path, probe_duration(path), probe_format_name(path), read_basic_tags(path), self._probe_sample_rate(path))); existing.add(path.resolve()); added += 1
        self._update_common_root(); self.refresh_table(); self._update_sample_rate_source_hint()
    ...
    def _analysis_finished(self, exit_code:int, _status) -> None:
        ...
        if exit_code==0:
            job.sample_rate = job.sample_rate or self._probe_sample_rate(job.source)
            self._update_sample_rate_source_hint()
    ...
    def _start_queue(self, rows:list[int], preview:bool) -> None:
        ...
        profile = self._current_quality_profile()
        self._queue_settings={
            "format_name": self.format_combo.currentText(),
            "quality_name": str(profile.get("core_quality", DEFAULT_QUALITY)),
            "quality_label": str(profile.get("label", self.quality_combo.currentText())),
            "quality_args": list(profile.get("ffmpeg_args", [])),
            "sample_rate_choice": self.sample_rate_combo.currentData() if hasattr(self, "sample_rate_combo") else "source",
        ...
        sr_choice = self._queue_settings.get("sample_rate_choice", "source")
        sr_text = "Wie Quelle/Analyse" if str(sr_choice) == "source" else self._format_sample_rate(int(sr_choice))
        self.log_message(f"Queue-Einstellungen: Preset={self._queue_settings['preset_name']}, Normalisieren={self._queue_settings['normalize']}, Format={self._queue_settings['format_name']}, Qualität={self._queue_settings['quality_label']}, Abtastrate={sr_text}")
    ...
    def _start_next_job(self) -> None:
        ...
        qual=str(self._queue_settings.get("quality_name", DEFAULT_QUALITY))
        quality_label=str(self._queue_settings.get("quality_label", qual))
        quality_args=list(self._queue_settings.get("quality_args", []))
        sample_rate_choice=self._queue_settings.get("sample_rate_choice", "source")
        ...
        try:
            spec=get_format_spec(fmt, qual)
        except Exception:
            spec=get_format_spec(fmt, DEFAULT_QUALITY)
        out_dir=Path(self.output_dir_edit.text()).expanduser(); out_dir.mkdir(parents=True, exist_ok=True)
        ...
        sample_rate = self._resolve_output_sample_rate(job, sample_rate_choice, fmt)
        output_audio_options = list(quality_args)
        if sample_rate:
            output_audio_options += ["-ar", str(sample_rate)]
        args = self._inject_output_audio_options(args, output_audio_options)
        self._current_stderr=[]
        sr_log = self._format_sample_rate(sample_rate) if sample_rate else "Encoder-Standard"
        self.log_message(f"Starte: {job.source.name} → {output.name} | Preset={preset}, Normalisieren={normalize}, Format={fmt}, Qualität={quality_label}, Abtastrate={sr_log}, Extra-Headroom={extra_headroom_db:.0f} dB")
